...
do_btrfs_snapshot() {
	##     $1 includes /current
	local SUBVOL="$1"
	local SNAPSHOT_ROOT="$(dirname ${SUBVOL})"

	echo "Creating a snapshot of $SUBVOL"
	if [[ -d "${SNAPSHOT_ROOT}" ]]; then
	        local SNAPSHOT_BASENAME="$(basename ${SNAPSHOT_ROOT})"
		local SNAPSHOT="${SNAPSHOT_ROOT}/@snapshot_${SNAPSHOT_BASENAME}_${DATE}"

		if [[ -d "${SNAPSHOT}" ]]; then
			 btrfs subvolume delete  "${SNAPSHOT}" && echo "old snapshot \'${SNAPSHOT}\' removed" 
		fi

		btrfs subvolume snapshot "${SUBVOL}" "${SNAPSHOT}" || echo "Error in creating btrfs snapshot $SNAPSHOT"  > /dev/stderr
	else
		echo "Cannot find snapshot base dir: ${DST_ROOT}" > /dev/stderr
		exit 5
	fi 
}
...
do_backup() {
	...
	local DST="${2:-$SRC}"
        DST="$(basename $DST)"
	local DST_FULL="${BACKUP_DIR}/${DST}/current"

	echo "SRC: $SRC"  | tee -a $TEMP
	echo "DST: ${DST_FULL}"  | tee -a $TEMP

	LOG=$(echo -n $SRC | tr /\  __ | sed s/^_//).log_${DATE} 
	...
	if [[ -d "${DST_FULL}" ]]; then
		...
		if  $(is_btrfs) ; then 
			...
			nice rsync -av --delete --delete-after "${SRC}/" "${DST_FULL}" | tee -a "/home/backup/backup_$LOG"  | tee -a $TEMP
			touch_stamp "$SRC"
			sync 
		else
			RSYNC_BACKUP_DIR="${BACKUP_ROOT}/${DST}/${DATE}/"
			## 2014-10-18: --progress remoevd to make logs more readable
                        nice rsync -abv --delete --delete-after --backup-dir="${RSYNC_BACKUP_DIR}" "${SRC}/" "${DST_FULL}" | tee -a "/home/backup/backup_$LOG"   | tee -a $TEMP
                        touch_stamp "$SRC"
                        sync
		fi
...
do_backup /home/stuff/Videos Videos
